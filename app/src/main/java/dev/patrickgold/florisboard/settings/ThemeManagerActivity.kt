...
class ThemeManagerActivity : FlorisActivity<ThemeManagerActivityBinding>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.setTitle(
            when (key) {
                prefs.theme.dayThemeRef.key() -> R.string.settings__theme_manager__title_day
                prefs.theme.nightThemeRef.key() -> R.string.settings__theme_manager__title_night
    ...
    private fun evaluateSelectedRef(ignorePrefs: Boolean = false): AssetRef? {
        return if (ignorePrefs) {
            when (key) {
                prefs.theme.dayThemeRef.key() -> themeManager.indexedDayThemeRefs.keys.firstOrNull()
                prefs.theme.nightThemeRef.key() -> themeManager.indexedNightThemeRefs.keys.firstOrNull()
                else -> null
            }
        } else {
            AssetRef.fromString(
                when (key) {
                    prefs.theme.dayThemeRef.key() -> prefs.theme.dayThemeRef.get()
                    prefs.theme.nightThemeRef.key() -> prefs.theme.nightThemeRef.get()
                    else -> ""
                }
            ).getOrDefault(null)
        }
    ...
    private fun setThemeRefInPrefs(ref: AssetRef?) {
        when (key) {
            prefs.theme.dayThemeRef.key() -> prefs.theme.dayThemeRef.set(ref.toString())
            prefs.theme.nightThemeRef.key() -> prefs.theme.nightThemeRef.set(ref.toString())
        }
    }
        ...
        when (view.id) {
            R.id.fab_option_create_empty -> {
                ...
                val newTheme = Theme.baseTheme(
                    ...
                    authors = listOf("@me"),
                    isNightTheme = key == prefs.theme.nightThemeRef.key()
                )
                val newAssetRef =
                    AssetRef(
                        AssetSource.Internal,
    ...
    private fun buildUi() {
        val metaIndex = when (key) {
            prefs.theme.dayThemeRef.key() -> themeManager.indexedDayThemeRefs
            prefs.theme.nightThemeRef.key() -> themeManager.indexedNightThemeRefs
            else -> mutableMapOf()
        }
